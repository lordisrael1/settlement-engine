...
--
-- Balance-zero, append-only and integer kobo are enforced here, in the database, rather
...
-- ---------------------------------------------------------------------------
-- Transactions. The primary key IS the causing event's idempotency key, so idempotency
...
-- ---------------------------------------------------------------------------
-- Entries. Money is BIGINT kobo (integer kobo) — never NUMERIC, never DOUBLE PRECISION.
...
-- ---------------------------------------------------------------------------
-- Balance-zero: — balance-zero, enforced as a DEFERRED constraint trigger.
--
...
-- ---------------------------------------------------------------------------
-- Append-only: — append-only. Correct a mistake with a compensating transaction.
--
-- account_balances is deliberately NOT protected: it is a cache, and a cache
-- that cannot be updated is not a cache. verifyBalances() is what keeps it honest.
-- ---------------------------------------------------------------------------
...
-- SUM(entries) for its account, and verifyBalances() asserts exactly that. When they
